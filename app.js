...
function safeEl(selector){const el=$(selector); if(!el) console.warn('Missing element',selector); return el;}
...
function renderAudit(){$('#audit').innerHTML=state.audit.slice(0,10).map(x=>`<tr><td>${x[0]}</td><td>${x[1]}</td><td><code>${x[2]}</code></td></tr>`).join('');$('#e-audit').textContent=state.audit.length;}
